...
import RadiusSelector from '@/components/properties/workspace/spyhunt/components/RadiusSelector.vue';
...
const radiusChangeHandler  = (value: number)  =>{
    emits('changed', {type: 'radius', value});
}
const emits = defineEmits<{
    (e: 'changed', payload: {type: string, value: any}): void
}>()
</script>

<template>
    <div
        class="relative h-[520px] w-full overflow-hidden rounded-[12px] bg-gradient-to-tr from-[#dfe3f8] via-[#eef1ff] to-white shadow-[0_25px_65px_rgba(82,90,186,0.2)]"
    >
        <div
        ...
        <!--                    {{ highlightComparable.squareFootage?.toLocaleString() ?? '—' }}-->
        <!--                    ft²-->
        <!--                </p>-->
        ...
        >
            <RadiusSelector
                ...
                :default-radius="filters.defaults.radius"
                @radius:changed="(e) => radiusChangeHandler(e)"
            />
            ...
            <!--                        @input="-->
            <!--                           handlePriceInput('min',Number(($event.target as HTMLInputElement).value),)"-->
            <!--                    />-->
            <!--                    <input-->
            <!--                        type="range"-->
            <!--                        class="h-1 flex-1 appearance-none rounded-full bg-gray-200 accent-[#7c4dff]"-->
